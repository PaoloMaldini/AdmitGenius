...
import com.admitgenius.util.AuthUtil; // Assuming you have a utility to get current user ID
...
public class SchoolManagementController {
    ...
    // but can be duplicated here if a different auth scheme or path is needed for
    // management.
    // For now, we assume they are publicly accessible or have their own auth in
    ...
    @PreAuthorize("hasAuthority('ADMIN')")
    public ResponseEntity<SchoolProgramDTO> addSchoolProgram(@PathVariable Long schoolId,
    ...
    @PreAuthorize("hasAnyAuthority('ADMIN', 'SCHOOL_ASSISTANT')")
    public ResponseEntity<SchoolProgramDTO> updateSchoolProgram(@PathVariable Long programId,
            @Valid @RequestBody SchoolProgramDTO programDTO) {
        Long currentUserId = authUtil.getCurrentUserId();
