...
import com.admitgenius.dto.EssayPolishDTO;
...
public class EssayPolishService {
    ...
    private String buildPolishPrompt(EssayPolishDTO polishDTO, String originalContent) {
        ...
        // Add customDescription to the prompt
        if (polishDTO.getCustomDescription() != null && !polishDTO.getCustomDescription().trim().isEmpty()) {
            prompt.append("\n## 用户具体要求\n");
            prompt.append(polishDTO.getCustomDescription().trim()).append("\n");
        }
    ...
    private String callOpenAIApi(String prompt) {
        try {
            ...
            System.out.println("API Key: "
                    + (openaiApiKey != null ? openaiApiKey.substring(0, Math.min(15, openaiApiKey.length())) + "..."
                            : "NULL"));
